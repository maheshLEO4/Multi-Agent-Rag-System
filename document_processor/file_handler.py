...
class DocumentProcessor:
    ...
    # ---------------- PROCESSING ---------------- #
    def process(self, files: List, batch_size: int = 32, chunk_size: int = 1200, chunk_overlap: int = 100):
        """Main processing pipeline: streaming ingestion to Qdrant."""
        ...
        for file in files:
            try:
                ...
                for chunk_batch in self._stream_chunks(file.name, chunk_size, chunk_overlap, batch_size):
                    # Insert batch into Qdrant immediately
    ...
    def _stream_chunks(
        ...
        """Yield embedded chunks in batches to avoid memory spikes."""
        # Load raw documents
    ...
    def _load_file(self, filename: str) -> List[Document]:
        """Load PDF or text files into Documents."""
        documents = []
