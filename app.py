...
class RetrieverWrapper:
    """Wraps a vector store to provide an invoke() method for workflow."""
    def __init__(self, vector_store):
        self.vector_store = vector_store

    def invoke(self, query: str):
        # Adjust k if needed
        return self.vector_store.similarity_search(query, k=20)
...
def main():
    ...
    with st.sidebar:
        ...
        if uploaded_files:
            ...
            # --- PROCESS DOCUMENTS INTO RETRIEVER ---
            ...
            if st.session_state.retriever is None or current_hashes != st.session_state.file_hashes:
                with st.spinner("🤖 Processing documents and updating retriever..."):
                    try:
                        ...
                        st.session_state.retriever = RetrieverWrapper(st.session_state.processor.vector_store)
